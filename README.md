...
The main goal was to implement and compare Breadth-First Search (BFS) and Depth-First Search (DFS) algorithms on a dataset that mimics real-world social interactions.
...
3. Number of mutual connections
...
* **`graph.py`**: Contains the main `Graph` class. This handles adding nodes, creating edges, and running the BFS/DFS logic.
...
* **BFS (Breadth-First Search):** Used to explore the immediate neighbors of a user first, expanding layer by layer. This is useful for finding closer connections.
* **DFS (Depth-First Search):** Used to explore a path as deep as possible before backtracking.
...
Bu proje, sosyal ağlardaki kullanıcı etkileşimlerini simüle etmek ve analiz etmek amacıyla Python ile geliştirilmiş bir ağırlıklı çizge (weighted graph) uygulamasıdır.

Kullanıcıları birer "düğüm" (node), aralarındaki ilişkileri ise "ağırlıklı kenar" (weighted edge) olarak modelleyen bu sistemde, Breadth-First Search (BFS) ve Depth-First Search (DFS) algoritmalarının davranışlarını incelemek hedeflenmiştir.

## Çalışma Mantığı

Standart "arkadaş mı, değil mi?" (0 veya 1) mantığının ötesine geçerek, bu projede her bağlantının bir kuvveti (weight) bulunur. İki kişi arasındaki bu bağın gücü şu üç kritere göre hesaplanır:
1. Aktiflik seviyeleri
2. Etkileşim sıklığı
3. Ortak bağlantı sayısı
...
* **DFS (Derinlik Öncelikli Arama):** Bir ilişki zincirini takip ederek gidebildiği en uç noktaya kadar ilerler, sonra geri döner (backtracking).
